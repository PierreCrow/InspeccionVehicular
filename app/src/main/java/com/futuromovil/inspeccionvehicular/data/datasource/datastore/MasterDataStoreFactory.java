package com.futuromovil.inspeccionvehicular.data.datasource.datastore;

import android.content.Context;

import com.futuromovil.inspeccionvehicular.data.datasource.cloud.store.CloudMasterDataStore;
import com.futuromovil.inspeccionvehicular.data.datasource.db.store.DbMasterDataStore;

public class MasterDataStoreFactory {

    public static final int DB = 1;
    public static final int CLOUD = 0;

    private final Context context;

    public MasterDataStoreFactory(Context context) {
        if (context == null) {
            throw new IllegalArgumentException("Context cannot be null!");
        }
        this.context = context.getApplicationContext();
    }


    public MasterDataStore create(int dataSource) {
        MasterDataStore usuarioDataStore = null;

        switch (dataSource) {
            case CLOUD:
                usuarioDataStore = createCloudDataStore();
                break;
            case DB:
                usuarioDataStore = new DbMasterDataStore(context);
                break;
        }
        return usuarioDataStore;
    }

    private MasterDataStore createCloudDataStore() {
        return new CloudMasterDataStore();
    }

}
